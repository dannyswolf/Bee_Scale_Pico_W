...
def sent_to_HiveMQ_Cloud(data_to_send):
    ...
    try:
        ...
        sleep(1)
        if DEBUG:
            print("Sending picow/battery_volts", data_to_send[5])
        mqtt_client.publish("picow/battery_volts", str(f"{data_to_send[5]}"))
        sleep(1)
        if DEBUG:
            print("Sending picow/shunt_voltage", data_to_send[6])
        mqtt_client.publish("picow/shunt_voltage", str(f"{data_to_send[6]}"))
        sleep(1)
        if DEBUG:
            print("Messages send")
